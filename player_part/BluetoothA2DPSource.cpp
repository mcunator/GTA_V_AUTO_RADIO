...
void BluetoothA2DPSource::forceStartMedia() {
    esp_a2d_cb_param_t fake;
    fake.media_ctrl_stat.cmd = ESP_A2D_MEDIA_CTRL_CHECK_SRC_RDY;
    fake.media_ctrl_stat.status = ESP_A2D_MEDIA_CTRL_ACK_SUCCESS;
    bt_app_av_media_proc(ESP_A2D_MEDIA_CTRL_ACK_EVT, &fake);
}
    ...
    switch (event) {
        ...
        case ESP_A2D_AUDIO_STATE_EVT:
            ...
            if(a2d->audio_stat.state == ESP_A2D_AUDIO_STATE_STARTED) {
                ...
                ESP_LOGI(BT_AV_TAG, "Audio suspended -> restarting");
                forceStartMedia();
            }
            // callback on state change
        ...
        case ESP_AVRC_TG_PASSTHROUGH_CMD_EVT: {
            ...
            esp_avrc_tg_cb_param_t *rc = (esp_avrc_tg_cb_param_t *)(p_param);
                ...
                if (rc->psth_cmd.key_code == ESP_AVRC_PT_CMD_PLAY &&
                    rc->psth_cmd.key_state == ESP_AVRC_PT_CMD_STATE_PRESSED) {
                    ESP_LOGW(BT_AV_TAG, "PLAY received -> forcing media start");
                    forceStartMedia();
                }
